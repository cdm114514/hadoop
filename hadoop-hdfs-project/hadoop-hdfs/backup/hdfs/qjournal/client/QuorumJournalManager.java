...
public class QuorumJournalManager implements JournalManager
{
    ...
    static String parseJournalId(URI uri)
    {
        ...
        Preconditions.checkArgument(path != null && !path.isEmpty(), "Bad URI '%s': must identify journal in path component", uri);
    ...
    {
        Preconditions.checkArgument(jid != null && !jid.isEmpty() && !jid.contains("/") && !jid.startsWith("."), "bad journal id: " + jid);
    }
    ...
    static List<AsyncLogger> createLoggers(Configuration conf, URI uri, NamespaceInfo nsInfo, AsyncLogger.Factory factory) throws IOException
    {
    ...
    private static List<InetSocketAddress> getLoggerAddresses(URI uri) throws IOException
    {
        ...
        {
            addrs.add(NetUtils.createSocketAddr(addr, DFSConfigKeys.DFS_JOURNALNODE_RPC_PORT_DEFAULT));
        }
        return addrs;
    }
    ...
    public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxnId, boolean inProgressOk) throws IOException
    {
        ...
        for (Map.Entry<AsyncLogger, RemoteEditLogManifest> e : resps.entrySet())
        {
            ...
            for (RemoteEditLog remoteLog : manifest.getLogs())
            {
                ...
                EditLogInputStream elis = EditLogFileInputStream.fromUrl(connectionFactory, url, remoteLog.getStartTxId(), remoteLog.getEndTxId(), remoteLog.isInProgress());
                allStreams.add(elis);
            }
        }
    ...
    public void doPreUpgrade() throws IOException
    {
        ...
        try
        {
            call.waitFor(loggers.size(), loggers.size(), 0, PRE_UPGRADE_TIMEOUT_MS, "doPreUpgrade");
    ...
    public void doRollback() throws IOException
    {
        ...
        try
        {
            call.waitFor(loggers.size(), loggers.size(), 0, ROLL_BACK_TIMEOUT_MS, "doRollback");
