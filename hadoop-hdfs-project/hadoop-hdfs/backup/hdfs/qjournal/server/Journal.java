...
public class Journal implements Closeable
{
    ...
    Journal(Configuration conf, File logDir, String journalId, StartupOption startOpt, StorageErrorReporter errorReporter) throws IOException
    {
    ...
    {
        LOG.info("Updating lastPromisedEpoch from " + lastPromisedEpoch.get() + " to " + newEpoch + " for client " + Server.getRemoteIp());
    ...
    private synchronized void checkWriteRequest(RequestInfo reqInfo) throws IOException
    {
        ...
        {
            throw new IOException("IPC's epoch " + reqInfo.getEpoch() + " is not the current writer epoch  " + lastWriterEpoch.get());
        }
    }
