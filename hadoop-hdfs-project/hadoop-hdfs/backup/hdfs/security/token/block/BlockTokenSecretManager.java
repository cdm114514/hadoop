...
public class BlockTokenSecretManager extends SecretManager<BlockTokenIdentifier>
{
    ...
    public synchronized ExportedBlockKeys exportKeys()
    {
        ...
        return new ExportedBlockKeys(true, keyUpdateInterval, tokenLifetime, currentKey, allKeys.values().toArray(new BlockKey[0]));
    }

    private synchronized void removeExpiredKeys()
    {
        long now = Time.now();
        for (Iterator<Map.Entry<Integer, BlockKey>> it = allKeys.entrySet().iterator(); it.hasNext(); )
        {
     ...
     */
    public void checkAccess(Token<BlockTokenIdentifier> token, String userId, ExtendedBlock block, AccessMode mode) throws InvalidToken
    {
     ...
     *
     * @param identifier the block token identifier
     ...
     *
     * @param identifier the block token identifier to look up
