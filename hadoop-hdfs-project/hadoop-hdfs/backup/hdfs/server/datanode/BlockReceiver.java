...
class BlockReceiver implements Closeable
{
    ...
    void receiveBlock(DataOutputStream mirrOut, // output to next datanode
                      ...
                      String mirrAddr, DataTransferThrottler throttlerArg, DatanodeInfo[] downstreams, boolean isReplaceBlock) throws IOException
    {
            ...
            { // Data transfer was not complete.
                if (responder != null)
                {
                    ...
                    if (datanode.isRestarting() && isClient && !isTransfer)
                    {
                        ...
                        try (Writer out = new OutputStreamWriter(new FileOutputStream(restartMeta), "UTF-8"))
                        {
    ...
    class PacketResponder implements Runnable, Closeable
    {
         ...
         */
        void sendOOBResponse(final Status ackStatus) throws IOException, InterruptedException
        {
            if (!running)
            {
    ...
    private static class Packet
    {
        ...
        Packet(long seqno, boolean lastPacketInBlock, long offsetInBlock, long ackEnqueueNanoTime, Status ackStatus)
        {
            this.seqno = seqno;
        ...
        public String toString()
        {
            return getClass().getSimpleName() + "(seqno=" + seqno + ", lastPacketInBlock=" + lastPacketInBlock + ", offsetInBlock=" + offsetInBlock + ", ackEnqueueNanoTime=" + ackEnqueueNanoTime + ", ackStatus=" + ackStatus + ")";
        }
    }
}
