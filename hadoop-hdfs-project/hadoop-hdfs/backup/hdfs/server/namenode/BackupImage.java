...
public class BackupImage extends FSImage
{
    ...
    private void logEditsLocally(long firstTxId, int numTxns, byte[] data)
    {
        ...
        Preconditions.checkState(firstTxId == expectedTxId, "received txid batch starting at %s but expected txn %s", firstTxId, expectedTxId);
    ...
    {
        Preconditions.checkArgument(firstTxId == lastAppliedTxId + 1, "Received txn batch starting at %s but expected %s", firstTxId, lastAppliedTxId + 1);
        ...
        try
        {
            ...
            long numTxnsAdvanced = logLoader.loadEditRecords(backupInputStream, true, lastAppliedTxId + 1, null, null);
            ...
            FSImage.updateCountForQuota(getNamesystem().dir.getBlockStoragePolicySuite(), getNamesystem().dir.rootDir); // inefficient!
        } finally
        {
    ...
    {
        Preconditions.checkState(bnState == BNState.JOURNAL_ONLY, "bad state: %s", bnState);
    ...
    private boolean tryConvergeJournalSpool() throws IOException
    {
        ...
        while (lastAppliedTxId < editLog.getCurSegmentTxId() - 1)
        {
            ...
            Iterable<EditLogInputStream> editStreamsAll = editLog.selectInputStreams(lastAppliedTxId, target - 1);
            // remove inprogress
        ...
        synchronized (this)
        {
            ...
            Collection<EditLogInputStream> editStreams = getEditLog().selectInputStreams(getEditLog().getCurSegmentTxId(), getEditLog().getCurSegmentTxId());
