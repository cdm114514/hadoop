...
class ClusterJspHelper
{
     ...
     * </ol>
     *
     * @param statusMap map whose key is datanode, value is an inner map with key being
    ...
    static class NamenodeMXBeanHelper
    {
        ...
        private static void getLiveNodeCount(String json, NamenodeStatus nn) throws IOException
        {
            ...
            for (Entry<String, Map<String, Object>> entry : nodeMap.entrySet())
            {
                ...
                if (innerMap != null)
                {
                    if (innerMap.get("adminState").equals(AdminStates.DECOMMISSIONED.toString()))
                    {
                        nn.liveDecomCount++;
                    }
                }
        ...
        private static void getDeadNodeCount(String json, NamenodeStatus nn) throws IOException
        {
            ...
            for (Entry<String, Map<String, Object>> entry : nodeMap.entrySet())
            {
                ...
                {
                    if (((Boolean) innerMap.get("decommissioned")).booleanValue() == true)
                    {
                        nn.deadDecomCount++;
                    }
                }
        ...
        private static void getDeadNodeStatus(Map<String, Map<String, String>> statusMap, String host, String json) throws IOException
        {
            ...
            for (Entry<String, Map<String, Object>> entry : nodeMap.entrySet())
            {
                ...
                if (deadNodeDetailMap != null && !deadNodeDetailMap.isEmpty())
                {
                    ...
                    if (((Boolean) deadNodeDetailMap.get("decommissioned")).booleanValue() == true)
                    {
                        deadDecommed.add(dn);
    ...
    static class DecommissionStatus
    {
        ...
        public void toXML(XMLOutputter doc) throws IOException
        {
            ...
            for (String dnhost : dnSet)
            {
                ...
                if (overallStatus != null && (overallStatus.equals(AdminStates.DECOMMISSION_INPROGRESS.toString()) || overallStatus.equals(AdminStates.DECOMMISSIONED.toString()) || overallStatus.equals(DecommissionStates.PARTIALLY_DECOMMISSIONED.toString()) || overallStatus.equals(DecommissionStates.UNKNOWN.toString())))
                {
                    doc.startTag("node");
                    // dn
                    toXmlItemBlockWithLink(doc, dnhost, new URL("http", dnhost, httpPort, ""), "DataNode");
        ...
        private void countDecommissionDatanodes()
        {
            for (String dn : statusMap.keySet())
            {
                ...
                {
                    decommissioned++;
                } else if (status.equals(DecommissionStates.DECOMMISSION_INPROGRESS.toString()))
                {
                    decommissioning++;
                } else if (status.equals(DecommissionStates.PARTIALLY_DECOMMISSIONED.toString()))
                {
                    partial++;
                }
            }
