...
public final class DirectoryWithQuotaFeature implements INode.Feature
{
    ...
    public static class Builder
    {
        ...
        public Builder()
        {
            this.quota = new QuotaCounts.Builder().nameSpace(DEFAULT_NAMESPACE_QUOTA).storageSpace(DEFAULT_STORAGE_SPACE_QUOTA).typeSpaces(DEFAULT_STORAGE_SPACE_QUOTA).build();
    ...
    ContentSummaryComputationContext computeContentSummary(final INodeDirectory dir, final ContentSummaryComputationContext summary)
    {
    ...
    private void checkStoragespace(final INodeDirectory dir, final long computed)
    {
        ...
        {
            NameNode.LOG.error("BUG: Inconsistent storagespace for directory " + dir.getFullPathName() + ". Cached = " + usage.getStorageSpace() + " != Computed = " + computed);
        }
    }

    void addSpaceConsumed(final INodeDirectory dir, final QuotaCounts counts, boolean verify) throws QuotaExceededException
    {
        if (dir.isQuotaSet())
        {
    ...
    private void verifyStoragespaceQuota(long delta) throws DSQuotaExceededException
    {
        ...
        {
            throw new DSQuotaExceededException(quota.getStorageSpace(), usage.getStorageSpace() + delta);
        }
    }
    ...
    boolean isQuotaSet()
    {
        return quota.anyNsSsCountGreaterOrEqual(0) || quota.anyTypeSpaceCountGreaterOrEqual(0);
    }
    ...
    {
        return "namespace: " + (quota.getNameSpace() < 0 ? "-" : usage.getNameSpace() + "/" + quota.getNameSpace());
    }

    private String storagespaceString()
    {
        return "storagespace: " + (quota.getStorageSpace() < 0 ? "-" : usage.getStorageSpace() + "/" + quota.getStorageSpace());
    }
    ...
    private String typeSpaceString()
    {
        ...
        for (StorageType t : StorageType.getTypesSupportingQuota())
        {
            sb.append("StorageType: " + t + (quota.getTypeSpace(t) < 0 ? "-" : usage.getTypeSpace(t) + "/" + usage.getTypeSpace(t)));
        }
        return sb.toString();
    }
