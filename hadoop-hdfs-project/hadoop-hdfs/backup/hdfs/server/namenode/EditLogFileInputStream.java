...
public class EditLogFileInputStream extends EditLogInputStream
{
     ...
     *
     * @param connectionFactory the URLConnectionFactory used to create the connection.
    ...
    private void init(boolean verifyLayoutVersion) throws LogHeaderCorruptException, IOException
    {
        ...
        try
        {
            ...
            if (NameNodeLayoutVersion.supports(LayoutVersion.Feature.ADD_LAYOUT_FLAGS, logVersion) || logVersion < NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION)
            {
                try
                {
    ...
    static int readLogVersion(DataInputStream in, boolean verifyLayoutVersion) throws IOException, LogHeaderCorruptException
    {
        ...
        if (verifyLayoutVersion && (logVersion < HdfsConstants.NAMENODE_LAYOUT_VERSION || // future version
                logVersion > Storage.LAST_UPGRADABLE_LAYOUT_VERSION))
        { // unsupported
            throw new LogHeaderCorruptException("Unexpected version of the file system log file: " + logVersion + ". Current version = " + HdfsConstants.NAMENODE_LAYOUT_VERSION + ".");
        }
        return logVersion;
    }
