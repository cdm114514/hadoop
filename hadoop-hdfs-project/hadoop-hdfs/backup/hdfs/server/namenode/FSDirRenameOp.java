...
class FSDirRenameOp
{
     ...
     */
    static void verifyFsLimitsForRename(FSDirectory fsd, INodesInPath srcIIP, INodesInPath dstIIP) throws PathComponentTooLongException, MaxDirectoryItemsExceededException
    {
     ...
     */
    @Deprecated
    static boolean unprotectedRenameTo(FSDirectory fsd, String src, String dst, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp) throws IOException
    {
     ...
     */
    static void renameTo(FSDirectory fsd, FSPermissionChecker pc, String src, String dst, BlocksMapUpdateInfo collectedBlocks, boolean logRetryCache, Options.Rename... options) throws IOException
    {
        final INodesInPath srcIIP = fsd.getINodesInPath4Write(src, false);
        final INodesInPath dstIIP = fsd.getINodesInPath4Write(dst, false);
    ...
    static boolean renameForEditLog(FSDirectory fsd, String src, String dst, long timestamp, Options.Rename... options) throws IOException
    {
        ...
        final INodesInPath srcIIP = fsd.getINodesInPath4Write(src, false);
        final INodesInPath dstIIP = fsd.getINodesInPath4Write(dst, false);
        boolean ret = unprotectedRenameTo(fsd, src, dst, srcIIP, dstIIP, timestamp, collectedBlocks, options);
     ...
     */
    static boolean unprotectedRenameTo(FSDirectory fsd, String src, String dst, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp, BlocksMapUpdateInfo collectedBlocks, Options.Rename... options) throws IOException
    {
        assert fsd.hasWriteLock();
    ...
    private static class RenameOperation
    {
        ...
        boolean removeSrc4OldRename()
        {
            ...
            if (removedSrc == -1)
            {
                NameNode.stateChangeLog.warn("DIR* FSDirRenameOp.unprotectedRenameTo: " + "failed to rename " + src + " to " + dst + " because the source" + " can not be removed");
                return false;
            } else
            {
                // update the quota count if necessary
        ...
        boolean cleanDst(BlockStoragePolicySuite bsps, BlocksMapUpdateInfo collectedBlocks) throws QuotaExceededException
        {
            Preconditions.checkState(oldDstChild != null);
            ...
            {
                filesDeleted = oldDstChild.cleanSubtree(bsps, Snapshot.CURRENT_STATE_ID, dstIIP.getLatestSnapshotId(), collectedBlocks, removedINodes).getNameSpace() >= 0;
            }
        ...
        void updateQuotasInSourceTree(BlockStoragePolicySuite bsps) throws QuotaExceededException
        {
            ...
            if (isSrcInSnapshot)
            {
                ...
                QuotaCounts newSrcCounts = srcChild.computeQuotaUsage(bsps, new QuotaCounts.Builder().build(), false);
