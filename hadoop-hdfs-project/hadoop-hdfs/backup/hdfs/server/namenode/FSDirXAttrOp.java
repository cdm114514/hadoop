...
class FSDirXAttrOp
{
    private static final XAttr KEYID_XATTR = XAttrHelper.buildXAttr(CRYPTO_XATTR_ENCRYPTION_ZONE, null);
    private static final XAttr UNREADABLE_BY_SUPERUSER_XATTR = XAttrHelper.buildXAttr(SECURITY_XATTR_UNREADABLE_BY_SUPERUSER, null);

    /**
     * Set xattr for a file or directory.
     *
     * @param src   - path on which it sets the xattr
     * @param xAttr - xAttr details to set
     * @param flag  - xAttrs flags
     * @throws IOException
     */
    static HdfsFileStatus setXAttr(FSDirectory fsd, String src, XAttr xAttr, EnumSet<XAttrSetFlag> flag, boolean logRetryCache) throws IOException
    {
     ...
     *
     * @param src   - path to remove the xattr from
     * @param xAttr - xAttr to remove
     * @throws IOException
     */
    static HdfsFileStatus removeXAttr(FSDirectory fsd, String src, XAttr xAttr, boolean logRetryCache) throws IOException
    {
        FSDirXAttrOp.checkXAttrsConfigFlag(fsd);
        FSPermissionChecker pc = fsd.getPermissionChecker();
        XAttrPermissionFilter.checkPermissionForApi(pc, xAttr, FSDirectory.isReservedRawName(src));
    ...
    static List<XAttr> unprotectedRemoveXAttrs(FSDirectory fsd, final String src, final List<XAttr> toRemove) throws IOException
    {
    ...
    static INode unprotectedSetXAttrs(FSDirectory fsd, final String src, final List<XAttr> xAttrs, final EnumSet<XAttrSetFlag> flag) throws IOException
    {
        assert fsd.hasWriteLock();
        INodesInPath iip = fsd.getINodesInPath4Write(FSDirectory.normalizePath(src), true);
        INode inode = FSDirectory.resolveLastINode(iip);
    ...
    static List<XAttr> getXAttrs(FSDirectory fsd, INode inode, int snapshotId) throws IOException
    {
        fsd.readLock();
        try
        {
    ...
    static XAttr unprotectedGetXAttrByName(INode inode, int snapshotId, String xAttrName) throws IOException
    {
    ...
    private static void checkXAttrChangeAccess(FSDirectory fsd, INodesInPath iip, XAttr xAttr, FSPermissionChecker pc) throws AccessControlException
    {
    ...
    private static List<XAttr> getXAttrs(FSDirectory fsd, String src) throws IOException
    {
