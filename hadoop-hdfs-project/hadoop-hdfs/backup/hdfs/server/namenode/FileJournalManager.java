...
public class FileJournalManager implements JournalManager
{
    ...
    private static final Pattern EDITS_INPROGRESS_STALE_REGEX = Pattern.compile(NameNodeFile.EDITS_INPROGRESS.getName() + "_(\\d+).*(\\S+)");

    private File currentInProgress = null;

    @VisibleForTesting
    StoragePurger purger = new NNStorageRetentionManager.DeletionStoragePurger();

    public FileJournalManager(Configuration conf, StorageDirectory sd, StorageErrorReporter errorReporter)
    {
        this.conf = conf;
        this.sd = sd;
     ...
     *
     * @param logDir directory to match edit logs in
     * @return matched edit logs
     * @throws IOException IOException thrown for invalid logDir
     */
        ...
        for (File f : filesInStorage)
        {
            ...
            if (inProgressEditsMatch.matches())
            {
                try
                {
                    ...
                    ret.add(new EditLogFile(f, startTxId, HdfsConstants.INVALID_TXID, true));
                    continue;
                } catch (NumberFormatException nfe)
                {
    ...
    @Override
    synchronized public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId, boolean inProgressOk) throws IOException
    {
    ...
    public static EditLogFile getLogFile(File dir, long startTxId) throws IOException
    {
        ...
        {
            throw new IllegalStateException("More than one log segment in " + dir + " starting at txid " + startTxId + ": " + Joiner.on(", ").join(ret));
        }
    }
    ...
    public static class EditLogFile
    {
        ...
        public String toString()
        {
            return String.format("EditLogFile(file=%s,first=%019d,last=%019d," + "inProgress=%b,hasCorruptHeader=%b)", file.toString(), firstTxId, lastTxId, isInProgress(), hasCorruptHeader);
        }
    }
