...
public abstract class INode implements INodeAttributes, Diff.Element<byte[]>
{
     ...
     *
     * @param latestInDst id of the latest snapshot in the DST tree above the reference node
     ...
     *                        file/directory.
     * @param priorSnapshotId The id of the latest snapshot before the to-be-deleted snapshot.
     ...
     * @param removedINodes   INodes collected from the descents for further cleaning up of
     *                        inodeMap
     * @return quota usage delta when deleting a snapshot
     */
    public abstract QuotaCounts cleanSubtree(final BlockStoragePolicySuite bsps, final int snapshotId, int priorSnapshotId, BlocksMapUpdateInfo collectedBlocks, List<INode> removedINodes);
     ...
     *                        inodeMap
     */
    public abstract void destroyAndCollectBlocks(BlockStoragePolicySuite bsps, BlocksMapUpdateInfo collectedBlocks, List<INode> removedINodes);
     ...
     */
    public abstract ContentSummaryComputationContext computeContentSummary(ContentSummaryComputationContext summary);
    ...
    {
        return new QuotaCounts.Builder().nameSpace(HdfsConstants.QUOTA_RESET).storageSpace(HdfsConstants.QUOTA_RESET).typeSpaces(HdfsConstants.QUOTA_RESET).build();
    }
    ...
    {
        final byte storagePolicyId = isSymlink() ? BlockStoragePolicySuite.ID_UNSPECIFIED : getStoragePolicyID();
        return computeQuotaUsage(bsps, storagePolicyId, new QuotaCounts.Builder().build(), true, Snapshot.CURRENT_STATE_ID);
    }
     ...
     */
    public abstract QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, QuotaCounts counts, boolean useCache, int lastSnapshotId);

    public final QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, QuotaCounts counts, boolean useCache)
    {
        final byte storagePolicyId = isSymlink() ? BlockStoragePolicySuite.ID_UNSPECIFIED : getStoragePolicyID();
        return computeQuotaUsage(bsps, storagePolicyId, counts, useCache, Snapshot.CURRENT_STATE_ID);
    }
    ...
    {
        return parent == null ? null : parent.isReference() ? getParentReference().getParent() : parent.asDirectory();
    }
    ...
    {
        byte localId = isSymlink() ? BlockStoragePolicySuite.ID_UNSPECIFIED : getLocalStoragePolicyID();
        return localId != BlockStoragePolicySuite.ID_UNSPECIFIED ? localId : parentStoragePolicyId;
    }
