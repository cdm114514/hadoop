...
public class INodesInPath
{
            ...
            } else if (isRef && isDir && !lastComp)
            {
                ...
                if (!isSnapshot)
                {
                    ...
                    if (snapshotId == CURRENT_STATE_ID || // no snapshot in dst tree of rename
                            (dstSnapshotId != CURRENT_STATE_ID && dstSnapshotId >= snapshotId))
                    { // the above scenario
    ...
    {
        return snapshotId == CURRENT_STATE_ID || (sid != CURRENT_STATE_ID && ID_INTEGER_COMPARATOR.compare(snapshotId, sid) < 0);
    }
    ...
    {
        Preconditions.checkArgument(offset >= 0 && length >= 0 && offset + length <= path.length);
