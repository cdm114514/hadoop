...
public class HftpFileSystem extends FileSystem implements DelegationTokenRenewer.Renewable, TokenAspect.TokenManagementDelegator
{
    ...
    class LsParser extends DefaultHandler
    {
        ...
        private void fetchList(String path, boolean recur) throws IOException
        {
            try
            {
                ...
                HttpURLConnection connection = openConnection("/listPaths" + ServletUtil.encodePath(path), "ugi=" + getEncodedUgiParameter() + (recur ? "&recursive=yes" : ""));
    ...
    private class ChecksumParser extends DefaultHandler
    {
        ...
        {
            final HttpURLConnection connection = openConnection("/fileChecksum" + ServletUtil.encodePath(f), "ugi=" + getEncodedUgiParameter());
            try
            {
    ...
    private class ContentSummaryParser extends DefaultHandler
    {
        ...
        {
            final HttpURLConnection connection = openConnection("/contentSummary" + ServletUtil.encodePath(path), "ugi=" + getEncodedUgiParameter());
            InputStream in = null;
            try
            {
