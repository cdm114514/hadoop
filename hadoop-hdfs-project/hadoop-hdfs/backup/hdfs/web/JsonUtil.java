...
public class JsonUtil
{
    ...
    public static HdfsFileStatus toFileStatus(final Map<?, ?> json, boolean includesType)
    {
        ...
        final long fileId = m.containsKey("fileId") ? ((Number) m.get("fileId")).longValue() : INodeId.GRANDFATHER_INODE_ID;
        final int childrenNum = getInt(m, "childrenNum", -1);
        final byte storagePolicy = m.containsKey("storagePolicy") ? (byte) ((Number) m.get("storagePolicy")).longValue() : BlockStoragePolicySuite.ID_UNSPECIFIED;
    ...
    private static LocatedBlock toLocatedBlock(final Map<?, ?> m) throws IOException
    {
        ...
        final LocatedBlock locatedblock = new LocatedBlock(b, locations, null, null, startOffset, isCorrupt, cachedLocations);
