...
class BlockReaderLocalLegacy implements BlockReader
{
    ...
    private static int getSlowReadBufferNumChunks(int bufferSizeBytes, int bytesPerChecksum)
    {
        ...
        {
            throw new IllegalArgumentException("Configured BlockReaderLocalLegacy " + "buffer size (" + bufferSizeBytes + ") is not large enough to hold " + "a single chunk (" + bytesPerChecksum + "). Please configure " + DFSConfigKeys.DFS_CLIENT_READ_SHORTCIRCUIT_BUFFER_SIZE_KEY + " appropriately");
        }
     ...
     * </ul>
     *
     * @param buf byte buffer to write bytes to. If checksums are not required, buf
     ...
     * read happens.
     *
     * @param len the maximum number of bytes to make available. After len bytes
