...
public class DistributedFileSystem extends FileSystem
{
    ...
    public void initialize(URI uri, Configuration conf) throws IOException
    {
        ...
        homeDirPrefix = conf.get(DFSConfigKeys.DFS_USER_HOME_DIR_PREFIX_KEY, DFSConfigKeys.DFS_USER_HOME_DIR_PREFIX_DEFAULT);
     ...
     * true.
     *
     * @param blocks List of target BlockLocations to query volume location information
    ...
    @Override
    protected HdfsDataOutputStream primitiveCreate(Path f, FsPermission absolutePermission, EnumSet<CreateFlag> flag, int bufferSize, short replication, long blockSize, Progressable progress, ChecksumOpt checksumOpt) throws IOException
    {
        statistics.incrementWriteOps(1);
    ...
    public void setStoragePolicy(final Path src, final String policyName) throws IOException
    {
        ...
        new FileSystemLinkResolver<Void>()
        {
            ...
            public Void next(final FileSystem fs, final Path p) throws IOException
            {
                ...
                {
                    throw new UnsupportedOperationException("Cannot perform setStoragePolicy on a non-DistributedFileSystem: " + src + " -> " + p);
                }
            }
        }.resolve(this, absF);
    }
    ...
    protected RemoteIterator<LocatedFileStatus> listLocatedStatus(final Path p, final PathFilter filter) throws IOException
    {
        ...
        {
            @Override
            public RemoteIterator<LocatedFileStatus> doCall(final Path p) throws IOException, UnresolvedLinkException
            {
     ...
     */
    private class DirListingIterator<T extends FileStatus> implements RemoteIterator<T>
    {
     ...
     * Enter, leave or get safe mode.
     *
     * @param action    One of SafeModeAction.ENTER, SafeModeAction.LEAVE and
     *                  SafeModeAction.GET
     * @param isChecked If true check only for Active NNs status, else check first NN's
     *                  status
    ...
    public SnapshotDiffReport getSnapshotDiffReport(final Path snapshotDir, final String fromSnapshot, final String toSnapshot) throws IOException
    {
        ...
        {
            @Override
            public SnapshotDiffReport doCall(final Path p) throws IOException, UnresolvedLinkException
            {
     ...
     * Add a cache pool.
     *
     * @param info The request to add a cache pool.
     ...
     *
     * @param info The request to modify a cache pool.
     * @throws IOException If the request could not be completed.
     */
     ...
     * Remove a cache pool.
     *
     * @param poolName Name of the cache pool to remove.
     * @throws IOException if the cache pool did not exist, or could not be removed.
     */
     ...
     * @throws IOException If there was an error listing cache pools.
     */
    ...
    public Map<String, byte[]> getXAttrs(Path path) throws IOException
    {
        ...
        return new FileSystemLinkResolver<Map<String, byte[]>>()
        {
            ...
            @Override
            public Map<String, byte[]> next(final FileSystem fs, final Path p) throws IOException, UnresolvedLinkException
            {
    ...
    public List<String> listXAttrs(Path path) throws IOException
    {
        ...
        return new FileSystemLinkResolver<List<String>>()
        {
            ...
            @Override
            public List<String> next(final FileSystem fs, final Path p) throws IOException, UnresolvedLinkException
            {
