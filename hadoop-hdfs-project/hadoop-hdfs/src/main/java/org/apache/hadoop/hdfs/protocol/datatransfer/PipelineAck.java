...
public class PipelineAck
{
    ...
    public enum ECN
    {
        DISABLED(0), SUPPORTED(1), SUPPORTED2(2), CONGESTED(3);
    ...
    {
        STATUS(null, 4), RESERVED(STATUS.BITS, 1), ECN_BITS(RESERVED.BITS, 2);
    ...
    static
    {
        ...
        String[] ele = conf.get(DFS_DATANODE_OOB_TIMEOUT_KEY, DFS_DATANODE_OOB_TIMEOUT_DEFAULT).split(",");
    ...
    public PipelineAck(long seqno, int[] replies, long downstreamAckTimeNanos)
    {
        ...
        proto = PipelineAckProto.newBuilder().setSeqno(seqno).addAllReply(statusList).addAllFlag(flagList).setDownstreamAckTimeNanos(downstreamAckTimeNanos).build();
    }
