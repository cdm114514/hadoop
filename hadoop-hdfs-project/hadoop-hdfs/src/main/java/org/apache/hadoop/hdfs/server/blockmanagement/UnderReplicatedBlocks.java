...
class UnderReplicatedBlocks implements Iterable<Block>
{
    ...
    synchronized boolean add(Block block, int curReplicas, int decomissionedReplicas, int expectedReplicas)
    {
        ...
        {
            if (priLevel == QUEUE_WITH_CORRUPT_BLOCKS && expectedReplicas == 1)
            {
                corruptReplOneBlocks++;
            }
            NameNode.blockStateChangeLog.debug("BLOCK* NameSystem.UnderReplicationBlock.add: {}" + " has only {} replicas and need {} replicas so is added to" + " neededReplications at priority level {}", block, curReplicas, expectedReplicas, priLevel);
    ...
    synchronized boolean remove(Block block, int oldReplicas, int decommissionedReplicas, int oldExpectedReplicas)
    {
        ...
        if (priLevel == QUEUE_WITH_CORRUPT_BLOCKS && oldExpectedReplicas == 1 && removedBlock)
        {
            corruptReplOneBlocks--;
            assert corruptReplOneBlocks >= 0 : "Number of corrupt blocks with replication factor 1 " + "should be non-negative";
        }
        return removedBlock;
    }
    ...
    boolean remove(Block block, int priLevel)
    {
        ...
        {
            NameNode.blockStateChangeLog.debug("BLOCK* NameSystem.UnderReplicationBlock.remove: Removing block {}" + " from priority queue {}", block, priLevel);
            return true;
        } else
        {
