...
public abstract class Storage extends StorageInfo
{
    ...
    private class DirIterator implements Iterator<StorageDirectory>
    {
        ...
        {
            StorageDirectory sd = getStorageDir(nextIndex);
            return (dirType == null || sd.getStorageDirType().isOfType(dirType)) && (includeShared || !sd.isShared());
        }
    }
    ...
    public static class StorageDirectory implements FormatConfirmable
    {
        ...
        public StorageState analyzeStorage(StartupOption startOpt, Storage storage) throws IOException
        {
            ...
            if (startOpt == HdfsServerConstants.StartupOption.FORMAT || (startOpt == StartupOption.HOTSWAP && hadMkdirs))
            {
            ...
            if (hasPreviousTmp)
            {
                if (hasPrevious)
                {
                    throw new InconsistentFSStateException(root, STORAGE_DIR_PREVIOUS + " and " + STORAGE_TMP_PREVIOUS + " cannot exist together.");
                }
                if (hasCurrent)
                {
            ...
            {
                throw new InconsistentFSStateException(root, "one and only one directory " + STORAGE_DIR_CURRENT + " or " + STORAGE_DIR_PREVIOUS + " must be present when " + STORAGE_TMP_REMOVED + " exists.");
            }
            if (hasCurrent)
            {
    ...
    public static boolean confirmFormat(Iterable<? extends FormatConfirmable> items, boolean force, boolean interactive) throws IOException
    {
        for (FormatConfirmable item : items)
        {
            ...
            { // Don't ask - always don't format
                System.err.println("Running in non-interactive mode, and data appears to exist in " + item + ". Not formatting.");
                return false;
            }
    ...
    public static void writeProperties(File to, StorageDirectory sd, Properties props) throws IOException
    {
    ...
    {
        return "NS-" + Integer.toString(storage.getNamespaceID()) + "-" + storage.getClusterID() + "-" + Long.toString(storage.getCTime());
    }
