...
public class FsVolumeImpl implements FsVolumeSpi
{
    ...
    private enum BlockFileFilter implements FilenameFilter
    {
        ...
        public boolean accept(File dir, String name)
        {
            return !name.endsWith(".meta") && name.startsWith(Block.BLOCK_FILE_PREFIX);
        }
    }
    ...
    private class BlockIteratorImpl implements FsVolumeSpi.BlockIterator
    {
        ...
        {
            List<String> children = IOUtils.listDirectory(dir, SubdirFilter.INSTANCE);
            cache = null;
            cacheMs = 0;
            if (children.size() == 0)
            {
                LOG.trace("getNextSubDir({}, {}): no subdirectories found in {}", storageID, bpid, dir.getAbsolutePath());
                return null;
            }
            ...
            if (nextSubDir == null)
            {
                LOG.trace("getNextSubDir({}, {}): no more subdirectories found in {}", storageID, bpid, dir.getAbsolutePath());
            } else
            {
                LOG.trace("getNextSubDir({}, {}): picking next subdirectory {} " + "within {}", storageID, bpid, nextSubDir, dir.getAbsolutePath());
            }
            return nextSubDir;
        }
        ...
        {
            File dir = Paths.get(bpidDir.getAbsolutePath(), "current", "finalized").toFile();
        ...
        public void load() throws IOException
        {
            ...
            LOG.trace("load({}, {}): loaded iterator {} from {}: {}", storageID, bpid, name, file.getAbsoluteFile(), mapper.writerWithDefaultPrettyPrinter().writeValueAsString(state));
        }
    ...
    void getVolumeMap(ReplicaMap volumeMap, final RamDiskReplicaTracker ramDiskReplicaMap) throws IOException
    {
        for (BlockPoolSlice s : bpSlices.values())
        {
    ...
    void getVolumeMap(String bpid, ReplicaMap volumeMap, final RamDiskReplicaTracker ramDiskReplicaMap) throws IOException
    {
