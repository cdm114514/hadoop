...
public final class CachedBlock implements Element, LightWeightGSet.LinkedElement
{
    ...
    public void removeInternal(IntrusiveCollection<? extends Element> list)
    {
        for (int i = 0; i < triplets.length; i += 3)
        {
            if (triplets[i] == list)
            {
                ...
                System.arraycopy(triplets, i + 3, newTriplets, i, triplets.length - (i + 3));
                triplets = newTriplets;
                return;
            }
        }
    ...
    {
        return new StringBuilder().append("{").append("blockId=").append(blockId).append(", ").append("replication=").append(getReplication()).append(", ").append("mark=").append(getMark()).append("}").toString();
    }
