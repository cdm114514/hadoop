...
class FSDirDeleteOp
{
     ...
     */
    static long delete(FSDirectory fsd, INodesInPath iip, BlocksMapUpdateInfo collectedBlocks, List<INode> removedINodes, long mtime) throws IOException
    {
     ...
     */
    static BlocksMapUpdateInfo delete(FSNamesystem fsn, String src, boolean recursive, boolean logRetryCache) throws IOException
    {
    ...
    static void deleteForEditLog(FSDirectory fsd, String src, long mtime) throws IOException
    {
        ...
        final INodesInPath iip = fsd.getINodesInPath4Write(FSDirectory.normalizePath(src), false);
     ...
     * For small directory or file the deletion is done in one shot.
     */
    static BlocksMapUpdateInfo deleteInternal(FSNamesystem fsn, String src, INodesInPath iip, boolean logRetryCache) throws IOException
    {
    ...
    private static boolean deleteAllowed(final INodesInPath iip, final String src)
    {
     ...
     */
    private static long unprotectedDelete(FSDirectory fsd, INodesInPath iip, BlocksMapUpdateInfo collectedBlocks, List<INode> removedINodes, long mtime)
    {
        assert fsd.hasWriteLock();
        ...
        {
            QuotaCounts counts = targetNode.cleanSubtree(fsd.getBlockStoragePolicySuite(), CURRENT_STATE_ID, latestSnapshot, collectedBlocks, removedINodes);
