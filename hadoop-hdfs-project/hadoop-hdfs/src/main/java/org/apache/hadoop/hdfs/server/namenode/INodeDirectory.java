...
public class INodeDirectory extends INodeWithAdditionalFields implements INodeDirectoryAttributes
{
    ...
    public byte getStoragePolicyID()
    {
        ...
        return getParent() != null ? getParent().getStoragePolicyID() : ID_UNSPECIFIED;
    }
    ...
    DirectoryWithQuotaFeature addDirectoryWithQuotaFeature(DirectoryWithQuotaFeature q)
    {
    ...
    public Snapshot addSnapshot(int id, String name) throws SnapshotException, QuotaExceededException
    {
        return getDirectorySnapshottableFeature().addSnapshot(this, id, name);
    }

    public Snapshot removeSnapshot(BlockStoragePolicySuite bsps, String snapshotName, BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) throws SnapshotException
    {
     ...
     */
    public INode saveChild2Snapshot(final INode child, final int latestSnapshotId, final INode snapshotCopy)
    {
     ...
     * @param snapshotId if it is not {@link Snapshot#CURRENT_STATE_ID}, get the result
    ...
    private QuotaCounts computeDirectoryQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, QuotaCounts counts, boolean useCache, int lastSnapshotId)
    {
        if (children != null)
        {
            for (INode child : children)
     ...
     * deleted list before.
     *
     * @param oldChild The reference node to be removed/replaced
     * @param newChild The node to be added back
     ...
     */
    public QuotaCounts cleanSubtreeRecursively(final BlockStoragePolicySuite bsps, final int snapshot, int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes, final Map<INode, INode> excludedNodes)
    {
    ...
    @Override
    public void destroyAndCollectBlocks(final BlockStoragePolicySuite bsps, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
    {
        final DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
        if (sf != null)
        {
    ...
    @Override
    public QuotaCounts cleanSubtree(final BlockStoragePolicySuite bsps, final int snapshotId, int priorSnapshotId, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
    {
        DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
    ...
    {
        return other != null && getQuotaCounts().equals(other.getQuotaCounts()) && getPermissionLong() == other.getPermissionLong() && getAclFeature() == other.getAclFeature() && getXAttrFeature() == other.getXAttrFeature();
    }
