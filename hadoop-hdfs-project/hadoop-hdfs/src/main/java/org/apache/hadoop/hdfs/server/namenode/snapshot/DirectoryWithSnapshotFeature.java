...
public class DirectoryWithSnapshotFeature implements INode.Feature
{
    ...
    static class ChildrenDiff extends Diff<byte[], INode>
    {
         ...
         */
        private boolean replace(final ListType type, final INode oldChild, final INode newChild)
        {
         ...
         */
        private QuotaCounts destroyCreatedList(final BlockStoragePolicySuite bsps, final INodeDirectory currentINode, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
        {
         ...
         */
        private QuotaCounts destroyDeletedList(final BlockStoragePolicySuite bsps, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
        {
            QuotaCounts counts = new QuotaCounts.Builder().build();
         ...
         */
        private void writeDeleted(DataOutput out, ReferenceMap referenceMap) throws IOException
        {
         ...
         * Serialize to out
         */
        private void write(DataOutput out, ReferenceMap referenceMap) throws IOException
        {
            writeCreated(out);
    ...
    public static class DirectoryDiff extends AbstractINodeDiff<INodeDirectory, INodeDirectoryAttributes, DirectoryDiff>
    {
        ...
        @Override
        QuotaCounts combinePosteriorAndCollectBlocks(final BlockStoragePolicySuite bsps, final INodeDirectory currentDir, final DirectoryDiff posterior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
        {
        ...
        @Override
        QuotaCounts destroyDiffAndCollectBlocks(BlockStoragePolicySuite bsps, INodeDirectory currentINode, BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
        {
    ...
    public static class DirectoryDiffList extends AbstractINodeDiffList<INodeDirectory, INodeDirectoryAttributes, DirectoryDiff>
    {
         ...
         */
        public boolean replaceChild(final ListType type, final INode oldChild, final INode newChild)
        {
        ...
        public int findSnapshotDeleted(final INode child)
        {
            ...
            {
                final ChildrenDiff diff = diffList.get(i).diff;
                final int d = diff.searchIndex(ListType.DELETED, child.getLocalNameBytes());
     ...
     */
    public static void destroyDstSubtree(final BlockStoragePolicySuite bsps, INode inode, final int snapshot, final int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) throws QuotaExceededException
    {
     ...
     */
    private static QuotaCounts cleanDeletedINode(final BlockStoragePolicySuite bsps, INode inode, final int post, final int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
    {
        QuotaCounts counts = new QuotaCounts.Builder().build();
     ...
     */
    public ReadOnlyList<INode> getChildrenList(INodeDirectory currentINode, final int snapshotId)
    {
        final DirectoryDiff diff = diffs.getDiffById(snapshotId);
        return diff != null ? diff.getChildrenList(currentINode) : currentINode.getChildrenList(Snapshot.CURRENT_STATE_ID);
    }

    public INode getChild(INodeDirectory currentINode, byte[] name, int snapshotId)
    {
        final DirectoryDiff diff = diffs.getDiffById(snapshotId);
        return diff != null ? diff.getChild(name, true, currentINode) : currentINode.getChild(name, Snapshot.CURRENT_STATE_ID);
    }

    /**
     * Used to record the modification of a symlink node
     */
    public INode saveChild2Snapshot(INodeDirectory currentINode, final INode child, final int latestSnapshotId, final INode snapshotCopy)
    {
    ...
    public void clear(BlockStoragePolicySuite bsps, INodeDirectory currentINode, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
    {
    ...
    public void computeContentSummary4Snapshot(final BlockStoragePolicySuite bsps, final ContentCounts counts)
    {
    ...
    public QuotaCounts cleanDirectory(final BlockStoragePolicySuite bsps, final INodeDirectory currentINode, final int snapshot, int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
    {
        QuotaCounts counts = new QuotaCounts.Builder().build();
        ...
        } else
        {
            ...
            counts.add(getDiffs().deleteSnapshotDiff(bsps, snapshot, prior, currentINode, collectedBlocks, removedINodes));
            counts.add(currentINode.cleanSubtreeRecursively(bsps, snapshot, prior, collectedBlocks, removedINodes, priorDeleted));
                ...
                if (priorDiff != null && priorDiff.getSnapshotId() == prior)
                {
                    ...
                    if (priorCreated != null)
                    {
                        ...
                        for (INode cNode : priorDiff.getChildrenDiff().getList(ListType.CREATED))
                        {
                            ...
                            {
                                counts.add(cNode.cleanSubtree(bsps, snapshot, Snapshot.NO_SNAPSHOT_ID, collectedBlocks, removedINodes));
                            }
                        }
                    }
