...
public class FSImageFormatPBSnapshot
{
    ...
    public final static class Loader
    {
        ...
        private void loadFileDiffList(InputStream in, INodeFile file, int size) throws IOException
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                for (int j = 0, e = bpl.size(); j < e; ++j)
                {
                    ...
                    if (storedBlock == null)
                    {
                        storedBlock = fsn.getBlockManager().addBlockCollection(new BlockInfoContiguous(blk, copy.getFileReplication()), file);
                    }
         ...
         */
        private List<INode> loadDeletedList(final List<INodeReference> refList, InputStream in, INodeDirectory dir, List<Long> deletedNodes, List<Integer> deletedRefNodes) throws IOException
        {
         ...
         */
        private void loadDirectoryDiffList(InputStream in, INodeDirectory dir, int size, final List<INodeReference> refList) throws IOException
        {
    ...
    public final static class Saver
    {
        ...
        public Saver(FSImageFormatProtobuf.Saver parent, FileSummary.Builder headers, SaveNamespaceContext context, FSNamesystem fsn)
        {
            this.parent = parent;
        ...
        public void serializeSnapshotSection(OutputStream out) throws IOException
        {
            ...
            for (INodeDirectory sdir : snapshottables)
            {
                for (Snapshot s : sdir.getDirectorySnapshottableFeature().getSnapshotList())
                {
                    ...
                    INodeSection.INodeDirectory.Builder db = buildINodeDirectory(sroot, parent.getSaverContext());
        ...
        private INodeReferenceSection.INodeReference.Builder buildINodeReference(INodeReference ref) throws IOException
        {
        ...
        private void saveCreatedList(List<INode> created, OutputStream out) throws IOException
        {
            ...
            for (INode c : created)
            {
                SnapshotDiffSection.CreatedListEntry.newBuilder().setName(ByteString.copyFrom(c.getLocalNameBytes())).build().writeDelimitedTo(out);
            }
        }

        private void serializeDirDiffList(INodeDirectory dir, final List<INodeReference> refList, OutputStream out) throws IOException
        {
            ...
            if (sf != null)
            {
                ...
                { // reverse order!
                    ...
                    {
                        db.setName(ByteString.copyFrom(copy.getLocalNameBytes())).setSnapshotCopy(buildINodeDirectory(copy, parent.getSaverContext()));
                    }
