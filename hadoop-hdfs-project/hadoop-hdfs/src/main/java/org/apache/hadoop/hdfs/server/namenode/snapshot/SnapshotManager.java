...
public class SnapshotManager implements SnapshotStatsMXBean
{
     ...
     * @throws IOException Throw IOException when the given path does not lead to an
     ...
     * @throws IOException Throw IOException when 1) the given path does not lead to an
     ...
     * @throws IOException
     */
    public void deleteSnapshot(final INodesInPath iip, final String snapshotName, BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) throws IOException
    {
        INodeDirectory srcRoot = getSnapshottableRoot(iip);
        srcRoot.removeSnapshot(fsdir.getBlockStoragePolicySuite(), snapshotName, collectedBlocks, removedINodes);
     ...
     *
     * @param oldSnapshotName Old name of the snapshot
     * @param newSnapshotName New name of the snapshot
     * @throws IOException Throw IOException when 1) the given path does not lead to an
     ...
     */
    public void renameSnapshot(final INodesInPath iip, final String snapshotRoot, final String oldSnapshotName, final String newSnapshotName) throws IOException
    {
    ...
    {
        return snapshottables.values().toArray(new INodeDirectory[snapshottables.size()]);
    }
