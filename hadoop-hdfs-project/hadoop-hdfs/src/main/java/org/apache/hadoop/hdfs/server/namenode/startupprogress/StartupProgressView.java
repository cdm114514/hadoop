...
public class StartupProgressView
{
    ...
    {
        return getElapsedTime(phases.get(Phase.LOADING_FSIMAGE), phases.get(Phase.SAFEMODE));
    }
    ...
    {
        StepTracking tracking = getStepTracking(phase, step);
        return tracking != null && tracking.total != Long.MIN_VALUE ? tracking.total : 0;
    }
    ...
    {
        final long elapsed;
        if (beginTracking != null && beginTracking.beginTime != Long.MIN_VALUE && endTracking != null && endTracking.endTime != Long.MIN_VALUE)
        {
            elapsed = endTracking.endTime - beginTracking.beginTime;
        } else if (beginTracking != null && beginTracking.beginTime != Long.MIN_VALUE)
        {
    ...
    private StepTracking getStepTracking(Phase phase, Step step)
    {
        ...
        Map<Step, StepTracking> steps = phaseTracking != null ? phaseTracking.steps : null;
